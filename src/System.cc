...
{
    // add the images
        // imshow image as bgr8
    cv::Mat originalImage = im.clone();
...
void System::SavePointcloud(const string &filename) {
    ...
    // Write header
    outFile << "# 3D point list with one line of data per point:" << endl;
    outFile << "#   POINT3D_ID, X, Y, Z, R, G, B, ERROR, TRACK[] as (IMAGE_ID, POINT2D_IDX)" << endl;
    outFile << "# Number of points: " << vpMapPoints.size() << ", mean track length: 0" << endl;
    ...
    for (MapPoint* pMP : vpMapPoints) {
        if (pMP && !pMP->isBad()) {
            ...
            // Find the first observing KeyFrame for this MapPoint
            const std::map<KeyFrame*, std::tuple<int, int>>& observations = pMP->GetObservations();
            cv::Vec3b color(0, 0, 0); // Default color
            bool foundColor = false;

            for (const auto& obs : observations) {
                KeyFrame* pKF = obs.first; // Get the observing KeyFrame
                if (pKF) {
                    // Extract the (u, v) coordinates
                    std::tuple<int, int> uv = obs.second;
                    int u = std::get<1>(uv);
                    int v = std::get<0>(uv);

                    // Check if the KeyFrame's RGB information is available
                    ...
                    if (it != mpTracker->mnIdToBGRMap.end()) {
                        ...
                        if (!bgrImage.empty() && bgrImage.rows > v && bgrImage.cols > u ) {
                            ...
                            // Write the 3D point and color information to the file
                            outFile << " " 
                                    << std::setprecision(20) << pos.x << " " << pos.y << " " << pos.z << " "
                                    << static_cast<int>(color[2]) << " " // Red
                                    << static_cast<int>(color[1]) << " " // Green
                                    << static_cast<int>(color[0]) << " "; // Blue // Error (default to 0)
                        
                            outFile << endl;
                            break; // Use the first valid KeyFrame's RGB
                        }
                    }
                }
            }
...
void System::SavePointcloudFromKeyframes(const std::string &filename) {
    ...
    const vector<KeyFrame*>& vpKeyFrames = pActiveMap->GetAllKeyFrames();

    // Write header
    outFile << "# 3D point list with one line of data per point:" << endl;
    outFile << "#   POINT3D_ID, X, Y, Z, R, G, B, ERROR, TRACK[] as (IMAGE_ID, POINT2D_IDX)" << endl;

    // Traverse all KeyFrames
    for (KeyFrame* pKF : vpKeyFrames) {
        if (!pKF || pKF->isBad()) continue;

        // Get the image and its corresponding map points
        auto it = mpTracker->mnIdToBGRMap.find(pKF->mnFrameId);
        if (it == mpTracker->mnIdToBGRMap.end() || it->second.empty()) {
            cerr << "Warning: No valid image for KeyFrame " << pKF->mnFrameId << endl;
            continue;
        }
        ...
        for (size_t i = 0; i < vpMapPoints.size(); ++i) {
            ...
            if (pMP && !pMP->isBad()) {
                // Get 3D position of the map point
                Eigen::Vector3f worldPos = pMP->GetWorldPos().cast<float>();
                cv::Point3f pos(worldPos.x(), worldPos.y(), worldPos.z());

                // Get the pixel coordinates of the map point in the KeyFrame
                const cv::KeyPoint& kp = pKF->mvKeys[i];
                int u = static_cast<int>(kp.pt.x);
                int v = static_cast<int>(kp.pt.y);

                // Ensure the pixel is within the image bounds
                if (u >= 0 && u < bgrImage.cols && v >= 0 && v < bgrImage.rows) {
                    // Extract RGB values
    ...
    // Close the file
    outFile.close();
    cout << "Pointcloud saved to " << filename << " using KeyFrame-based method." << endl;
}
